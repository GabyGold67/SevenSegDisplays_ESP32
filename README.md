...
|Method | Parameters|
|---|---|
|**_SevenSeg74HC595_** |uint8_t **sclk**, uint8_t **rclk**, uint8_t **dio**(, bool **commAnode**(, uint8_t **dspDigits**))|
|**begin()**|None|
|**blink()**|None|
...
### **SevenSeg74HC595**(uint8_t **sclk**, uint8_t **rclk**, uint8_t **dio**, bool **commAnode**, uint8_t **dspDigits**)
### Description:  
...
### Use example:  
**`SevenSeg74HC595 myLedDisp(6, 7, 10);`**  
**`SevenSeg74HC595 myLedDisp(6, 7, 10, false);`**  // Create a common cathode display  
**`SevenSeg74HC595 myLedDisp(6, 7, 10, true, 8);`**  // Create a common anode, 8 digits display  
...
### Parameters:  
**onRate**: unsigned long integer containing the time (in milliseconds) the display must stay on, the value must be in the range _minBlinkRate <= onRate <= _maxBlinkRate. Those preset values can be known by the use of the **`getMinBlinkRate()`** and the **`getMaxBlinkRate()`** methods.  
...
**`myLedDisp.blink(getMaxBlinkRate() + 10);`** //Returns false and the display stays without change.  
...
### **isWaiting**();
### Description:
Returns a boolean value indicating if the display is set to "waiting mode" or not. This might be useful as the display is unavailable to other **`.print()`**, **`write()`**, **`gauge()`**, etc. methods while it's on "waiting" mode.
### Parameters:  
**None**  
### Return value:  
...
### Parameters:  
**onRate**: unsigned long integer containing the time (in milliseconds) the display must stay on, the value must be in the range _minBlinkRate <= onRate <= _maxBlinkRate. Those built-in values can be known by the use of the **`getMinBlinkRate()`** and the **`getMaxBlinkRate()`** methods.  
...
### Return value:  
true: If the parameter or parameters passed are whitin the valid range, and the change will take effect immediately.
